...
<head>
    ...
    <title>403 - Restricted Access | {{ config('app.name', 'CribSearch') }}</title>
...
<body class="bg-bg text-dark font-sans min-h-screen flex flex-col justify-between selection:bg-primary/20 selection:text-primary">
    ...
    <main class="flex-grow flex items-center justify-center px-4 py-12">
        <div class="max-w-md w-full text-center space-y-6">
            ...
            <div class="inline-flex items-center justify-center w-20 h-20 bg-red-500/10 text-danger rounded-3xl mb-2">
            ...
            <div class="space-y-2">
                <span class="text-xs font-black tracking-widest uppercase text-danger">Error 403</span>
                <h1 class="text-3xl sm:text-4xl font-black tracking-tight text-dark uppercase">Access Restricted</h1>
                <p class="text-xs sm:text-sm text-slate-500 leading-relaxed pt-1">
                    {{ $exception->getMessage() ?: "You don't have permission to view this section or scout portal resource." }}
                </p>
            </div>
